...
- [0015 — Sales Factory C1: contract retrieval](0015-sales-factory-c1.md) — 🛠️ **BUILT, QC requested**: ingest contract `.md` (MOU/SOW/proposal/invoice from 3 deals) → `documents`/`document_chunks`; migration `0012` `search_docs` RPC (UNAPPLIED, clones recall_memory, metadata-only, service_role-only); `/api/search-docs` endpoint (clones recall.ts) + Documents dashboard tab. Dry-run 12 docs/0 quarantined; build + leak-clean. **First contract content in the brain** — apply + ingestion HELD for Aegis + Jesse go. Text-search only (Storage→C1b).
