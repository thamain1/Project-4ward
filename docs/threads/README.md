...
- [0013 — Unit B+.2: memory_entries tags](0013-memory-tags.md) — 🛠️ **BUILT, QC requested**: additive migration `0011` (UNAPPLIED) adds `tags text[]` + GIN; deterministic backfill (`project:`/`repo:`/`topic:`/`reusable`/`code-snippet`) for exact grouping + repo card badges + a code-snippet library; frontend tags-aware (deploys AFTER 0011 applies). Helios refinement pass proposed for nuanced code-snippet/applies-to tags. Gated: Aegis → apply 0011 → backfill → deploy frontend.
