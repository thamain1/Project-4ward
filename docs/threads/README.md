...
- [0009 — Secrets vault backend + `get_secret`](0009-secrets-vault-getsecret.md) — 📐 **DESIGN PROPOSAL** (no code): backend = **Supabase Vault** (Jesse's decision; native, installed, no new infra). Migration `0010` rewires `secrets_vault` to vault-backed encryption-at-rest + `set_secret`/`get_secret` rewrite; MCP `get_secret` tool follows as a separate slice. 6 design Qs for Aegis. Build gated on design sign-off.
