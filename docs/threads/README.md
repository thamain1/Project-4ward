...
- [0007 — MCP `remember` (write slice)](0007-mcp-remember.md) — **REMEDIATED r1**: Aegis rejected v1 (provenance overwrite / deferred audit / unbounded fan-out); redesigned → distinct `mcp/<slug>` provenance + no-overwrite collision policy + **atomic `remember_memory` RPC** (migration `0009`) + bounds. remember 60/0; **not run live**, `0009` UNAPPLIED, awaiting Aegis re-review.
