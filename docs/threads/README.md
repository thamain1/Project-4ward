...
- [0005 — Frontmatter backfill (Helios)](0005-frontmatter-backfill.md) — ✅ **17/17 BACKFILLED + LIVE** (2026-06-15): Helios classified all 17, Aegis security-reviewed, Atlas backfilled 16 then redacted + ingested `intellitax.md` as the 17th (brain **118 entries / 81 chunks**). `intellitax.md` also held a service-role + anon key — redacted from the local copy; key never left the machine (not in git/GitHub, never sent to API), IntelliTax unaffected; rotation precautionary only. Aegis reconciliation review pending.
