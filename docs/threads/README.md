...
- [0010 — MCP `get_secret` tool](0010-mcp-get-secret.md) — 🛠️ **BUILT, QC requested** (thread 0010 ≠ migration 0010; no new migration). Thin client over the audited/sensitivity-gated `get_secret_operator`; local-operator only, value never logged. getsecret 17/0 + regressions green; **not run live** pending Aegis sign-off → then smoke test. Completes the MCP toolset (recall/remember/log_update/get_secret).
