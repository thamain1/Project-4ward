...
- [0012 — Phase 2 / Unit B: dashboard semantic recall](0012-dashboard-recall.md) — 🛠️ **BUILT, QC requested**: first server-side endpoint (`functions/api/recall.ts` CF Pages Function) — JWT verify → active-member check → Gemini embed → `recall_memory` RPC; Memories page gets real semantic search (text filter kept as quick filter). No new migration. Build + `dist/` leak-clean (service-role absent, functions/ not bundled). **Endpoint not run live** — gated on Aegis QC + Jesse adding CF Function env (`SUPABASE_SERVICE_ROLE_KEY`, `GEMINI_API_KEY`). Foundation for the MOU/SOW sales factory.
