...
- [0007 — MCP `remember` (write slice)](0007-mcp-remember.md) — **REMEDIATED r3**: r2 cleared collision/byte/entity_type; fixed 2 fail-closed items (ownership predicate `~ '^mcp/'`; unified SQL secret pattern incl. `xox…`). remember 60/0 + log 34/0 + recall 27/0; **not run live**, `0009` UNAPPLIED, awaiting Aegis r3 re-review. (Aegis said: after these, approves applying `0009` solely to run the post-apply gate.)
