...
- [0008 — MCP `log_update` (DESIGN PROPOSAL)](0008-mcp-log-update.md) — 📐 **design only, no code**: proposes `log_activity` definer fn (migration `0009`) + `log_update` tool to append to `activity_log` (the audit/append primitive `remember` reuses). Submitted for Aegis design review alongside `0007` (shares the per-write-audit question). Build gated on design sign-off.
