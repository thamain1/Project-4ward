...
- [0007 — MCP `remember` (write slice)](0007-mcp-remember.md) — **REMEDIATED r2**: r1 architecture accepted; fixed 3 hardening mismatches (atomic + **bidirectional** collision guard in both write RPCs, `entity_type` secret-scan, UTF-8-byte `detail` bound). remember 60/0 + log 33/0 + recall 27/0; **not run live**, `0009` UNAPPLIED, awaiting Aegis r2 re-review.
