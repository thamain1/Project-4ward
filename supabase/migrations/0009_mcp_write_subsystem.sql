...
begin
  ...
  if p_entity_type is not null and length(p_entity_type) > 100 then raise exception 'log_activity: entity_type too long'; end if;
  -- detail: top-level object, bounded size/keys, FLAT (no nested object/array), bounded string values
  ...
  if length(p_detail::text) > 4096 then raise exception 'log_activity: detail exceeds 4KB'; end if;
...
begin
  ...
  -- NO-SILENT-OVERWRITE collision policy: never replace a file-backed canonical entry of the same name
  if exists (select 1 from public.memory_entries where name = v_name and source_path ~ '^memory/') then
    raise exception 'remember_memory: name "%" collides with a file-backed canonical memory; choose a different name', v_name;
  end if;
